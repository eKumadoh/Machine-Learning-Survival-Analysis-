...
final_tree_mod <- finalize_workflow(tree_wf, tree_hyper |>
                                     transmute(
                                       cost_complexity = cost_complexity_median,
                                       tree_depth = tree_depth_median,
                                       min_n = min_n_median
                                       )) |> 
  fit(data = breast_cancer) |> 
...
#Models run
# - Penalized proportional hazard 
# - Random forest
# - XGBoost
# - Decision tree

# Random forest model turned out to be the best
...
#I shall send the calibration plot sound 
